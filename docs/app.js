(function () {
  ...
  // ----------------------------------------------------------------------------
  // Vec2
  const distance$1 = ([x1, y1], [x2, y2]) => Math.sqrt(Math.pow(x1 - x2, 2) + Math.pow(y1 - y2, 2));
  ...
  const svg$1 = select('svg');
  const render = (data, width, height) => {
      svg$1.attr('width', width).attr('height', height);
      const svgNode = svg$1.node(0);
      ...
      const g = svg$1.selectAll('g.squares').data([1]).enter()
